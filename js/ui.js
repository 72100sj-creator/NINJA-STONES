/**
 * ui.js
 * Gestion exclusive de l'affichage (DOM). Ne calcule aucune logique métier.
 */
window.NS_UI = (function() {
    // Cache des éléments DOM pour éviter de les chercher à chaque fois
    let dom = {};
    let tilesElements = {}; // Stocke les éléments HTML des pierres
    ...
    function updateGardenVisual(boardEl, stage) {
        ...
        for (let i = 2; i <= C.MAX_VISUAL_STAGE; i++) {
            boardEl.classList.remove(C.CSS_STAGE_PREFIX + i);
        }
    ...
    function renderMenu(state, gardenConfig) {
        ...
        tilesElements = {};
        let boardWidth = dom.menuBoard.clientWidth;
        _renderStones(dom.menuBoard, NS_Puzzle.generateSolvedGrid(state.totalTiles), state.gridSize, boardWidth, null);
    }

    function renderGameBoard(grid, gridSize, onTileClickCallback) {
        dom.board.innerHTML = '';
        tilesElements = {};
        let boardWidth = dom.board.clientWidth;
        _renderStones(dom.board, grid, gridSize, boardWidth, onTileClickCallback);
    }
    ...
    function _renderStones(containerEl, grid, gridSize, boardWidth, onClickCallback) {
        ...
        for (let i = 0; i < grid.length; i++) {
            ...
            if (onClickCallback) {
                stone.addEventListener('click', () => onClickCallback(value));
            }
    ...
    function showWinMessage(text) {
        ...
        void dom.message.offsetWidth; // Force le repaint
    ...
    return {
        ...
        updateGardenVisual: updateGardenVisual,
        renderMenu: renderMenu,
        renderGameBoard: renderGameBoard,
        moveTile: moveTile,
        showWinMessage: showWinMessage,
        resetGameUI: resetGameUI,
        getDomElements: function() { return dom; } // Au cas où app.js en a besoin
    };
})();